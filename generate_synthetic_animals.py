        ...
        for video in range(num_videos):
            ...
            #            animal.set_material(1, animal_texture_fur)
                ...
                if sum(kpts[kp_18_id,2])>=4:
                    ...
                    sequence_dir_filename = os.path.join(sequence_output_dir, filename)
                    filename_mask = filename.replace(".png", "_mask.png")
                    filename_mask_whitened = filename.replace(".png", "_mask_white.png")
...
if __name__ == "__main__":
    ...
    frame_names, masked_frames, whitened_frames, extracted_kpts, fid, vids = retrieve(animal=args.animal, num_videos=args.videos, num_images=args.num_imgs, use_random_texture=args.use_random_texture)

    # Save as Metadatset
    META_DIR = f"synthetic_animals/{args.animal}s_meta/"
    ...
    dict_file = {
        'description': f"Synthetic animals - {args.animal}",
        'loaders': { 
            'frames':'image'},
        'loader_kwargs': {
            'frames': {
