...
class HourglassNet(nn.Module):
    ...
    def __init__(self, config):  # block, num_stacks=2, num_blocks=4, num_classes=20):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_residual(self.block, self.inplanes, 1)
        ...
        ch = self.num_feats * self.block.expansion
        hg, res, fc, score, fc_, score_ = [], [], [], [], [], []
        for i in range(self.num_stacks):
            hg.append(Hourglass(self.block, self.num_blocks, self.num_feats, 4))
            res.append(self._make_residual(self.block, self.num_feats, self.num_blocks))
            fc.append(self._make_fc(ch, ch))
            score.append(nn.Conv2d(ch, self.num_classes, kernel_size=1, bias=True))
            if i < self.num_stacks - 1:
                fc_.append(nn.Conv2d(ch, ch, kernel_size=1, bias=True))
                score_.append(nn.Conv2d(self.num_classes, ch, kernel_size=1, bias=True))
