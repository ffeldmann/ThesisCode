...
class ResnetTorchVision(nn.Module):
    def __init__(self, config):
        ...
        resnet = getattr(models, "resnet" + str(config.get("resnet_type","50")))(pretrained=config.get("pretrained", False))
        ...
        self.model.add_module("Up", nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                      nn.ReLU(inplace=True),
                                      nn.Conv2d(128, config["n_classes"], kernel_size=3, stride=1, padding=1)
                                      )
                              )
