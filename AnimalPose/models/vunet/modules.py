...
flags.DEFINE_float("dropout_prob", 0.0, "The dropout probability")
...
class VUnetResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.dout = nn.Dropout(p=FLAGS.dropout_prob)
        ...
        if self.gated:
            ...
            self.dout2 = nn.Dropout(p=FLAGS.dropout_prob)
            self.sigm = nn.Sigmoid()
        if activate:
            self.act_fn = nn.LeakyReLU() if FLAGS.final_act else nn.ELU()
        else:
            self.act_fn = IDAct()
