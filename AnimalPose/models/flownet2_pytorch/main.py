...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--number_gpus", "-ng", type=int, default=-1, help="number of GPUs to use"
    )
    parser.add_argument("--no_cuda", action="store_true")

    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument(
        "--name", default="run", type=str, help="a name to append to the save directory"
    )
    parser.add_argument(
        "--save", "-s", default="./work", type=str, help="directory for saving"
    )

    parser.add_argument(
        "--validation_frequency", type=int, default=5, help="validate every n epochs"
    )
    ...
    parser.add_argument(
        "--save_flow", action="store_true", help="save predicted flows to file"
    )
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        args.effective_inference_batch_size = (
            args.inference_batch_size * args.number_gpus
        )
    ...
    def train(
        ...
        if is_validate:
            ...
            args.validation_n_batches = (
                np.inf if args.validation_n_batches < 0 else args.validation_n_batches
            )
            progress = tqdm(
            ...
            args.train_n_batches = (
                np.inf if args.train_n_batches < 0 else args.train_n_batches
            )
            progress = tqdm(
                tools.IteratorTimer(data_loader),
                ncols=120,
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if args.cuda and args.number_gpus == 1:
                data, target = (
                    [d.cuda(async=True) for d in data],
            ...
            if not is_validate and args.fp16:
                ...
                for i in range(len(params)):
                    param_copy[i].grad = (
                        params[i].grad.clone().type_as(params[i]).detach()
                    )
            ...
            ) or (is_validate and batch_idx == args.validation_n_batches - 1):

                global_iteration = (
                    global_iteration if not is_validate else start_iteration
                )
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        args.inference_n_batches = (
            np.inf if args.inference_n_batches < 0 else args.inference_n_batches
        )
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.inference_batch_size):
                    ...
                    flow_utils.writeFlow(
                        join(
                            flow_folder,
                            "%06d.flo" % (batch_idx * args.inference_batch_size + i),
                        ),
                        _pflow,
                    )
    ...
    for epoch in progress:
        if args.inference or (
            args.render_validation and ((epoch - 1) % args.validation_frequency) == 0
        ):
            stats = inference(
                args=args,
                epoch=epoch - 1,
