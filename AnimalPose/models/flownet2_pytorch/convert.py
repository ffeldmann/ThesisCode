...
if "FlowNet2/" in args.caffe_model:
    ...
    parse_flownetfusion(
        model.flownetfusion.modules(), weights, biases, param_prefix="fuse_"
    )

    state = {"epoch": 0, "state_dict": model.state_dict(), "best_EPE": 1e10}
    torch.save(
        state, os.path.join(args.flownet2_pytorch, "FlowNet2_checkpoint.pth.tar")
    )
    ...
    state = {"epoch": 0, "state_dict": model.state_dict(), "best_EPE": 1e10}
    torch.save(
        state, os.path.join(args.flownet2_pytorch, "FlowNet2-C_checkpoint.pth.tar")
    )
    ...
    state = {"epoch": 0, "state_dict": model.state_dict(), "best_EPE": 1e10}
    torch.save(
        state, os.path.join(args.flownet2_pytorch, "FlowNet2-CS_checkpoint.pth.tar")
    )
    ...
    state = {"epoch": 0, "state_dict": model.state_dict(), "best_EPE": 1e10}
    torch.save(
        state, os.path.join(args.flownet2_pytorch, "FlowNet2-CSS_checkpoint.pth.tar")
    )
    ...
    torch.save(
        state,
        os.path.join(args.flownet2_pytorch, "FlowNet2-CSS-ft-sd_checkpoint.pth.tar"),
    )
    ...
    state = {"epoch": 0, "state_dict": model.state_dict(), "best_EPE": 1e10}
    torch.save(
        state, os.path.join(args.flownet2_pytorch, "FlowNet2-S_checkpoint.pth.tar")
    )
    ...
    state = {"epoch": 0, "state_dict": model.state_dict(), "best_EPE": 1e10}
    torch.save(
        state, os.path.join(args.flownet2_pytorch, "FlowNet2-SD_checkpoint.pth.tar")
    )

else:
    print(
        (
            "model type cound not be determined from input caffe model %s"
            % (args.caffe_model)
        )
    )
    quit()
