...
def add_arguments_for_module(
    parser, module, argument_for_class, default, skip_params=[], parameter_defaults={}
):
    ...
    for i, arg in enumerate(args):
        ...
        if arg not in skip_params + ["self", "args"]:
            if arg in list(parameter_defaults.keys()):
                ...
                print(
                    (
                        "[Warning]: non-default argument '{}' detected on class '{}'. This argument cannot be modified via the command line".format(
                            arg, module.__class__.__name__
                        )
                    )
                )
...
def kwargs_from_args(args, argument_for_class):
    ...
    return {
        ...
        if argument_for_class in key and key != argument_for_class + "class"
    }
...
def gpumemusage():
    gpu_mem = (
        subprocess.check_output("nvidia-smi | grep MiB | cut -f 3 -d '|'", shell=True)
        .replace(" ", "")
        .replace("\n", "")
        .replace("i", "")
    )
    ...
    for i in range(len(all_stat) / 2):
        ...
        cmem = str(int(math.ceil(curr / 1024.0))) + "GB"
        gmem = str(int(math.ceil(tot / 1024.0))) + "GB"
