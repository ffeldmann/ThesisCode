...
class StaticCenterCrop(object):
    ...
    def __call__(self, img):
        return img[
            (self.h - self.th) // 2 : (self.h + self.th) // 2,
...
class MpiSintel(data.Dataset):
    def __init__(self, args, is_cropped=False, root="", dstype="clean", replicates=1):
        ...
        if (
            ...
            or (self.render_size[1] < 0)
            or (self.frame_size[0] % 64)
            or (self.frame_size[1] % 64)
        ):
...
class FlyingChairs(data.Dataset):
    def __init__(
        self, args, is_cropped, root="/path/to/FlyingChairs_release/data", replicates=1
    ):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
...
class ChairsSDHomTest(ChairsSDHom):
    def __init__(self, args, is_cropped=False, root="", replicates=1):
        super(ChairsSDHomTest, self).__init__(
            args, is_cropped=is_cropped, root=root, dstype="test", replicates=replicates
        )
